...
fo_1_ex = 2.4e9
fo_2_ex = 2.4835e9
fo_1__fo_2 = fo_1_ex/fo_2_ex

f_o = fo_1 = 300e6
omega_o_1 = 2 * np.pi * fo_1

fo_2 = fo_1 * fo_1__fo_2
omega_o_2 = np.pi * 2 * fo_2
...
tuning_range = fo_1 / tuning_range_percent
Vdd = 2
KVCO = Kvco = fo_1/tuning_range_percent # Hz/V
...
I_CP_ang = C_1 * 2 * math.pi * N1 * omega_n**2 / Kvco_ang / A0
I = I_CP = C_1 * 2 * math.pi * N1 * omega_n**2 / Kvco / A0
...
R_1 = R1 = 2 * zeta * math.sqrt(1 / (K * C1))
C_2 = C2 = C_1 / 10 
```
...
num_O = np.convolve(H_sys.num, [N1])
...
num_den = np.convolve([1/N1], Kvco_over_s[0])
...
#Kvco_over_s = ([Kvco], [1, 0])
Kvco_over_s = ([Kvco], [1])
den_num = np.convolve([1/N1], Kvco_over_s[0])
...
t_step = sim_stop_time / 2  # instante do step
...
final_value = ((f_r+step_m_lin) * N1 - fo_1) / KVCO
#final_value = 0.5
...
# Valor final (assumido ou estimado)
final_value = ((f_r+step_m_lin) * N1 - fo_1) / KVCO
final_value *= KVCO / (step_m_lin_meas * N1)
#final_value = 0.5
...
| $K_\mathrm{VCO}$ | VCO sensivity    | `{python} format(Kvco)`Hz/V                     |
