...
For instance, to synthesize a frequency of `{python} format(f_example_1)`Hz, the required control voltage $V_c$ is computed using:
...
such that $A_0$ is the DC gain (usually unity), $K_\theta = I / 2\pi$ is the charge pump gain, $K_\mathrm{VCO}$ is the VCO gain, and $N$ is the frequency divider. Furthermore, to align the calculated values with the responses obtained in simulation, an empirical value for the DC gain was determined as $A_0 =$ `{python} round(A0, 2)`.
...
Thus, the resistor value is $R=$ `{python} format(R_1)`$\Omega$. Using a common design heuristic, the second capacitor $C_2$ is:
