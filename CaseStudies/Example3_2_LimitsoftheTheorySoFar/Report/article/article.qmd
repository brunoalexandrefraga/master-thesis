...
The loop filter transfer function, $F(s)$, neglecting the shunt capacitor, $C_2$ can be described as:

$$
F(s) = R+\frac{1}{sC_1}=\frac{sC_1R+1}{sC_1}
$$

Ainda ignorando o segundo capacitor, é possível escrever a função de transferência de malha aberta desse sistema apresentado na [Fig. @fig-PLL-integerN-blocks] como
...
tal que $A_0$ é o ganho DC, $K_\theta=I/2\pi$ o ganho do charge pump, $K_\mathrm{VCO}$ o ganho do VCO e $N$ o divisor de frequência.

A função de transferência em malha fechada é dada por

$$
\begin{split}
H(s)&=\frac{\theta_o}{\theta_R}=\frac{G(s)}{1+G(s)}\implies\\
&=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{N}\left(R+\frac{1}{sC_1}\right)}{s+\frac{A_0K_\theta K_\mathrm{VCO}}{N}\left(R+\frac{1}{sC_1}\right)}
\end{split}
$$ {#eq-PLL-closed-loop-transfer-function-initial}



That can be rearanged as:

$$
H(s)=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{2\pi\cdot NC_1}\left(RC_1s+1\right)}{s^2+\frac{A_0K_\theta IK_\mathrm{VCO}}{2\pi\cdot N}Rs+\frac{A_0K_\theta IK_\mathrm{VCO}}{2\pi\cdot NC_1}}
$$ {#eq-PLL-closed-loop-transfer-function-modified}
...
Com o filtro simplificado, sem o secundo capacitor,

$$
H(s)=\frac{\frac{K}{C_1}\left(RC_1s+1\right)}{s^2+KRs+\frac{K}{C_1}}
$$

Using the parameters $\omega_n$ and $\zeta$, the closed-loop transfer function of the entire loop can be described as (neglecting the second capacitor, $C_2$, of the filter which would yield to a not wished third-order transfer function):

$$
H(s)=\frac{\omega_n^2\left(\frac{2\zeta}{\omega_n}s+1\right)}{s^2+2\zeta\omega_n s+\omega_n^2}
$$ {#eq-PLL-closed-loop-transfer-function-zeta-omega_n}
...
For simplicity, the second capacitor can be obtained by $C_2=C_1/10$. Se esse segundo capacitor não fosse desconsiderado, os passos aqui apresentados seriam um pouco mais complicados. Entretanto, ainda it's possible to present the transfer function of the system in terms of all components values. This transfer function can be seen below (this equation uses the second capacitor, $C_2$, of the loop filter, adding a high-frequency pole to the system):
...
where $C_s$ is the paralel between $C_1$ and $C_2$. 
...
Utilizando as expressões [Eq. @eq-PLL-closed-loop-transfer-function-zeta-omega_n] e [Eq. @eq-PLL-closed-loop-transfer-function-modified],

$$
\begin{split}
E(s)&=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{NC_1}\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}\cdot\frac{1}{1/N}\cdot\frac{1}{A_0K_\mathrm{VCO}/s}\cdot\frac{1}{F(s)}\cdot\frac{1}{K_\theta}\implies\\
&=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{NC_1}\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}\cdot N\cdot\frac{s}{A_0K_\mathrm{VCO}}\cdot\frac{sC_1}{RC_1s+1}\cdot\frac{1}{K_\theta}
\end{split}
$$
...
$$
E(s)=\frac{s^2}{s^2+2\zeta\omega_ns+\omega_n^2}
...
\end{split}
$$ {#eq-transfer-function-step-frequency-phase-error}

Getting the inverse Laplace transform of [Eq. @eq-transfer-function-step-frequency-phase-error], for $\zeta<1$, results in:
...
The goal is to find the maximum input frequency step, $\Delta\omega_m$, such that the phase error does not exceed the maximum phase error that the PFD can withstand. Usually, this value is about $2\pi$. Using the normalized result:
...
$$
V(s)=\frac{V_C}{\theta_R}=H(s)\cdot\frac{1}{1/N}\cdot\frac{1}{K_\mathrm{VCO}/s}
...
$$
\begin{split}
V(s)&=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{NC_1}\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}\cdot\frac{1}{1/N}\cdot\frac{1}{K_\mathrm{VCO}/s}\implies\\
&=\frac{\frac{A_0K_\theta K_\mathrm{VCO}}{NC_1}\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}\cdot N\cdot\frac{s}{K_\mathrm{VCO}}
\end{split}
$$
...
$$
V(s)=\frac{\frac{A_0K_\theta}{C_1}s\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}
$$ {#eq-voltage-control-transfer-function}


Multiplicando o step de frequência nessa função, é obtido

$$
\begin{split}
V_C(s)&=\frac{\Delta\omega\frac{A_0K_\theta}{C_1s}\left(RC_1s+1\right)}{s^2+2\zeta\omega_ns+\omega_n^2}
\end{split}
$$ {#eq-transfer-function-step-frequency-voltage-control}
...
With the PLL values properly defined, the transfer function of this system is given by:
...
With the PLL values properly defined, the closed-loop transfer function of this system is given by:
