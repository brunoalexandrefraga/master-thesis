...
A [Fig. @fig-PLL-integerN-blocks] apresenta o diagrama de blocos de um PLL Integer-N PFD/CP-based.
...
tal que $A_0$ é o ganho DC (usualmente unitário), $K_\theta=I/2\pi$ o ganho do charge pump, $K_\mathrm{VCO}$ o ganho do VCO e $N$ o divisor de frequência. A partir daí, é possível obter a função de transferência em malha fechada do sistema, dada por

$$
H(s)=\frac{\theta_\mathrm{fb}}{\theta_R}=\frac{\omega_\mathrm{fb}}{\omega_R}=\frac{G(s)}{1+G(s)}
$$ {#eq-tf-closed-loop}

É possível observar que a função de transferência $H(s)$ apresentada na [Eq. @eq-PLL-closed-loop-transfer-function-initial] vale tanto para a fase do sinal de feedback pela fase do sinal de referência quanto pela frequência do sinal de feedback pela frequência do sinal de referência.
...
Neste momento, o interesse é obter uma equação que forneça o maior step de frequência que o PLL pode lidar. Para isso, analisa-se a equação do erro de fase e buscamos pelo valor de pico do erro de fase. Analysing the maximum of the [Eq. @eq-inverse-laplace-phase-error], the time instant at which the maximum phase error occurs, $t_m$, can be computed using:

$$
t_m=\frac{1}{\omega_n\sqrt{1-\zeta^2}}\tan^{-1}\left(\frac{\sqrt{1-\zeta^2}}{\zeta}\right)
$$ {#eq-instant-maximum-frequency-step}

[Eq. @eq-instant-maximum-frequency-step] represents the instant at which [Eq. @eq-inverse-laplace-phase-error] reaches its maximum value. Sendo a máxima normalized phase error,
...
A função de transferência da fase de saída pela fase de referência pode ser obtido como
...
Com os parâmetros $\zeta$ e $\omega_n$ definidos, é possível calcular o instante em que ocorre a máxima diferença de fase. Obtém-se, assim, $t_m=$ `{python} format(t_m)`s.
...
Normalmente, espera-se que o máximo erro de fase suportado pelo PFD seja de $2\pi$. Entretanto, para coincidir com os valores obtidos com o simulador utilizado, empiricamente obtém-se um maximum phase error, $\theta_{e_m}=$ `{python} format_with_pi_latex(theta_e_m)`.
...
Utilizando a [Eq. @eq-tf-filter], é possível obter the transfer function of the filter is given by:
...
Utilizando a [Eq. @eq-tf-open-loop], é possível obter the open-loop transfer function of the system is given by:
...
Utilizando a [Eq. @eq-tf-closed-loop], é possível obter the closed-loop transfer function of the entire system, both in frequency domain and phase domain is given by:
...
Utilizando a [Eq. @eq-tf-output-phase], é possível obter the transfer function of the output phase is given by:
...
Utilizando a [Eq. @eq-tf-phase-error], é possível obter the transfer function of the phase error is given by:
...
Utilizando a [Eq. @eq-tf-vctrl-vco], é possível obter the transfer function of the voltage control is given by:
...
A [Fig. @fig-pll-step-response] apresenta um step em frequência de `{python} format(step_amplitude)`Hz aplicado a função de transferência da [Eq. @eq-tf-closed-loop]
...
A step with the same amplitude as the previously calculated frequency step was applied in [Fig. @fig-pll-step-response].
