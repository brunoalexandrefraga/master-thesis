...
plt.axvline(ts_graphical * 1e6, color='green', linestyle='--', linewidth=1.2, label=f'Settling time (gráfico): {ts_graphical*1e6:.2f} ns')
plt.xlabel('Time [ns]')
...
## TODO: Analyze control voltage to verify if cycle slipping occurs if $\Delta\omega>\Delta\omega_m$
...
Com a intenção de se realizar testes para verificar a ocorrência de cycle slipping, estima-se a mínima frequência do sinal de referência. A ratio often quoted as being safe is 10:1 for $\omega_r/\omega_n$, dessa forma, a mínima frequência segura para o sinal de referência é dado por $f_r=$ `{python} format(f_r_safe)`Hz. A mínima frequência permitida, é dada por:
...
ou seja, a mínima frequência de referência é $f_r\ge$ `{python} format(f_r_max)`Hz. Dessa forma, como é desejada uma PLL operando com frequência de referência $f_r=$ `{python} format(f_r)`Hz e frequência de saída `{python} format(f_o)`Hz, o sistema projetado deve operar corretamente.
