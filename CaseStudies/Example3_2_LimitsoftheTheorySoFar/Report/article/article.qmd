...
The [Fig. @fig-PLL-integerN-blocks] presents the block diagram of an Integer-N PFD/CP-based PLL.

![Block diagram of Integer-N](../images/PLL_integerN_blocks.png){#fig-PLL-integerN-blocks}

The second-order loop filter transfer function, $F(s)$ can be described as:

$$
F(s) = \left(R+\frac{1}{sC_1}\right)\parallelsum\frac{1}{sC_2}
$$ {#eq-tf-filter}

The open-loop transfer function of the system shown in \[Fig. @fig-PLL-integerN-blocks] can be written as follows:
...
such that $A_0$ is the DC gain (usually unity), $K_\theta = I / 2\pi$ is the charge pump gain, $K_\mathrm{VCO}$ is the VCO gain, and $N$ is the frequency divider. From there, it is possible to obtain the closed-loop transfer function of the system, given by:

$$
H(s)=\frac{\theta_\mathrm{fb}}{\theta_R}=\frac{\omega_\mathrm{fb}}{\omega_R}=\frac{G(s)}{1+G(s)}
$$ {#eq-tf-closed-loop}

It can be observed that the transfer function $H(s)$ presented in \[Eq. @eq-PLL-closed-loop-transfer-function-initial] applies both to the phase of the feedback signal relative to the phase of the reference signal and to the frequency of the feedback signal relative to the frequency of the reference signal.

## Parameters and Components

The system parameters can be approximately calculated as follows:
...
The transfer function of the phase error relative to the reference phase can be obtained as:
...
At this point, the objective is to obtain an equation that provides the maximum frequency step the PLL can handle. To do so, the phase error equation is analyzed, and the peak value of the phase error is sought. Analyzing the maximum of [Eq. @eq-inverse-laplace-phase-error], the time instant at which the maximum phase error occurs, $t_m$, can be computed using:
...
[Eq. @eq-instant-maximum-frequency-step] represents the instant at which [Eq. @eq-inverse-laplace-phase-error] reaches its maximum value. Where the maximum normalized phase error is:
...
Leading to a maximum step frequency equal to:
...
The transfer function of the control voltage relative to the reference phase can be obtained as:

$$
V_C(s)=\frac{V_C}{\theta_R}=H(s)\cdot\frac{1}{1/N}\cdot\frac{1}{K_\mathrm{VCO}}
$$ {#eq-tf-vctrl-vco}

## Output phase Transfer Function

The transfer function of the output phase relative to the reference phase can be obtained as:

$$
O(s)=\frac{V_C}{\theta_R}=H(s)\cdot\frac{1}{1/N}
$$ {#eq-tf-output-phase}
...
To ensure the theoretical models are valid, the minimum frequency of the reference signal must be respected. A commonly cited safe ratio is 10:1 for $\omega_r / \omega_n$. However, the minimum allowed frequency is given by:
...
If this rule is not followed, the linear models cannot be guaranteed, requiring that signal sampling is also taken into account and the system be treated as a discrete system, as it truly is.
...
Based on the parameters already presented in the introduction, the natural frequency is $\omega_n = $ `{python} format_with_pi_latex(omega_n)`rad/s.
...
With the parameters $\zeta$ and $\omega_n$ defined, it is possible to calculate the instant at which the maximum phase difference occurs. Thus, $t_m =$ `{python} format(t_m)`s.
...
Normally, the maximum phase error supported by the PFD is expected to be $2\pi$. However, to match the values obtained with the simulator used, the maximum phase error is empirically determined as $\theta_{e_m} =$ `{python} format_with_pi_latex(theta_e_m)`.
...
With this frequency step and all parameters calculated so far, it is possible to compute the 1% settling time, resulting in $t_s =$ `{python} format(t_s)`s.
...
To illustrate the results obtained so far, an integer-N Phase-Locked Loop (PLL) was developed with a division factor of $N =$ `{python} N`. The system was designed with a VCO gain of $K_\mathrm{VCO} =$ `{python} format(KVCO)`Hz/V, and a charge pump current of $I =$ `{python} format(I)`A. The phase detector gain is given by $K_\theta = I / 2\pi$. Furthermore, to align the calculated values with the responses obtained in simulation, an empirical value for the DC gain was determined as $A_0 =$ `{python} round(A0, 2)`.
...
As shown in [Eq. @eq-tf-filter], the transfer function of the filter is given by:
...
The open-loop transfer function of the system is defined in [Eq. @eq-tf-open-loop], as follows:
...
As illustrated in [Eq. @eq-tf-closed-loop], the closed-loop transfer function of the system can be expressed as:
...
The transfer function of the output phase is provided in [Eq. @eq-tf-output-phase], as follows:
...
The transfer function of the phase error is presented in [Eq. @eq-tf-phase-error], described by:
...
As detailed in [Eq. @eq-tf-vctrl-vco], the transfer function of the control voltage of the VCO is given by:
...
As shown in [Fig. @fig-step-freq-h], a frequency step input of `{python} format(step_amplitude)`Hz is applied to evaluate the dynamic behavior of the closed-loop system described by the transfer function in [Eq. @eq-tf-closed-loop-numeric]. This analysis reveals the system’s capacity to reach a new steady-state condition in response to abrupt input changes and the settling time.
...
As illustrated in [Fig. @fig-ramp-phase-h], a phase ramp input is employed to assess the tracking capabilities of the closed-loop system represented by [Eq. @eq-tf-closed-loop]. This test simulates a constant frequency offset and allows for the evaluation of the system’s ability to maintain phase lock.
...
[Fig. @fig-ramp-phase-h-o] presents a comparative analysis of the phase responses of the closed-loop and output-phase transfer functions, as defined in [Eq. @eq-tf-closed-loop] and [Eq. @eq-tf-output-phase], respectively. The figure highlights their individual contributions to the system’s overall phase behavior under ramp input conditions.
...
In [Fig. @fig-ramp-phase-step-freq-h], a ramp input with a superimposed frequency step is used to analyze the transient behavior of the closed-loop system, whose response is characterized by [Eq. @eq-tf-closed-loop]. This approach reveals how the system adapts to dynamic input variations.
...
plt.axvline(t_step * 1e6, color='purple', linestyle='--', linewidth=1.0, label='Step Frequency Instant')
...
As depicted in [Fig. @fig-ramp-phase-h-phase-error], the response of the closed-loop system under a ramp input is evaluated with particular attention to the resulting phase error. The system behavior follows the model described by [Eq. @eq-tf-closed-loop], and this evaluation helps quantify the residual phase deviation.
...
# Plotagem
plt.figure(figsize=(8, 6))
plt.plot(t_out * 1e6, phi_in, linestyle=':', color='black', linewidth=0.8, label='Reference Phase')
...
The evolution of the phase error in response to a frequency step input is shown in [Fig. @fig-step-freq-e], and it corresponds to the transfer function presented in [Eq. @eq-tf-phase-error]. This analysis highlights the loop's capacity to promptly correct for frequency disturbances.
...
[Fig. @fig-step-freq-e-norm] illustrates the normalized version of the phase error response, which is governed by the transfer function in [Eq. @eq-tf-phase-error]. Normalization enables meaningful comparisons across different system designs.
...
# Linhas horizontais em ±1% do valor de pico (em torno de zero)
plt.axhline(tol, color='purple', linestyle='--', linewidth=1.0)
...
As can be observed in [Fig. @fig-step-freq-vc], the control voltage applied to the VCO is analyzed following a step in input frequency. The dynamic behavior is described by the transfer function in [Eq. @eq-tf-vctrl-vco], which reflects the loop's ability to adjust its frequency through the VCO.
...
[Fig. @fig-step-freq-vc-norm] presents the normalized control voltage response governed by [Eq. @eq-tf-vctrl-vco]. The normalized plot facilitates the comparison of different control loop designs by removing absolute voltage scale dependencies.
...
plt.axhline(final_value, color='red', linestyle='--', linewidth=0.8, label='Reference Voltage')
...
As illustrated in the Bode diagram shown in [Fig. @fig-bode-g], the open-loop transfer function of the system, defined in [Eq. @eq-tf-open-loop], characterizes the frequency response in terms of gain and phase. This representation enables the analysis of system stability and the determination of key dynamic properties such as gain margin and phase margin.
...
The frequency-domain characteristics of the closed-loop system, modeled by [Eq. @eq-tf-closed-loop], are depicted in [Fig. @fig-bode-h]. This Bode plot provides insight into stability margins, bandwidth, and overall system dynamics.
...
In [Fig. @fig-bode-f], the gain and phase of the loop filter, as defined in [Eq. @eq-tf-filter], are plotted. This representation is key for understanding how the filter shapes the loop response and contributes to noise suppression and stability.
