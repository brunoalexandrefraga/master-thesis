...
Além disso, a [Fig. @fig-phase-domain-system] apresenta dois outros ganhos que não foram muito apresentados, o ganho $A_0$ e $K_\theta$ ou $K_\text{phase}$. O ganho $A_0$ representa o ganho em corrente contínua e é assumido unitário nas simulações deste trabalho. O outro elemento de ganho, $K_\theta$, por sua vez é utilizado para quantizar a quantidade de corrente fornecida pelo charge pump durante um ciclo de operação. Se o charge pump conduziu pelo ciclo completo, a corrente média, $i_d=I$, se o charge pump conduziu por meio ciclo, $i_d=I/2$ e assim por diante. O que define o ângulo de condução do charge pump é a diferença de fase, $\theta_R-\theta_o$. Se a diferença for de $90^\circ$ ($\pi/2$), o carge pump vai conduzir por 1/4 do período e, portanto, $i_d=I/4$. Por isso, se tomarmos $\tau$ como o tempo de condução e $T$ o período, a definição da corrente média, $i_d$, é dada por @rogers2006IntegratedCircuitDesign
