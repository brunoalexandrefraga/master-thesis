...
abstract: |
    This report explores the linearity limits of Phase-Frequency Detectors (PFDs) in Phase-Locked Loops (PLLs) and the onset of cycle slipping, a nonlinear phenomenon that occurs when the phase error exceeds the detector's effective operating range. The analysis focuses on tristate PFDs commonly used with charge pumps, emphasizing their functional behavior and limitations. A practical methodology for estimating the linear range through simulation is presented, along with a discussion on how exceeding this range can lead to loss of phase tracking and degraded loop performance. The consequences of cycle slipping, including increased jitter and potential loss of lock, are also examined.
keywords: 
    - Fractional-N PLL
    - Linearity Limit
    - Cycle Slipping
