...
So the maximum frequency step at the input such that the theory so far is still able to predict the behavior of the system is `{python} format(step_m_lin)`Hz. If the frequency step is any larger than this, then the PLL will lose lock and cycle slip, which is the 

At other side, the settling time, $t_s$, can be calculated as
...
resulting in a settling time of $t_s=$`{python} format(t_s)`s.
